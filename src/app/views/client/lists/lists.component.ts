...
export class ListsComponent implements OnInit {
  ...
  addNewModalRef: ListFormComponent;
  showModal: any;
  ...
  loadData(pageSize, currentPage, direction, orderBy, search) {
    ...
    this.listsService
      .getLists(currentPage, orderBy, direction, pageSize, search)
      .subscribe(
        (data) => {
          if (data.status) {
      ...
      this.listsService.updateList(object).subscribe(
        (resCreate) => {
          this.notifications.create(
            "Success",
            "Supprimer l'articles avec succès",
  ...
  goToDetailsNewPage(list_id) {
    const url = this.router.serializeUrl(
      this.router.createUrlTree(["client/lists/all-articles"], {
        queryParams: { id: list_id },
      })
    );
