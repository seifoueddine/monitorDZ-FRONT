...
export class DetailsArticleComponent implements OnInit {
      ...
      (params) => {
        ...
        this.articlesService.getArticleById(this.articleId).subscribe(
          (res: any) => {
            ...
            this.article.attributes.body =
              '<div dir="auto" >' + this.article.attributes.body + "</div>";
            this.tags = res.tags;
            this.mediaName = res.article.data.attributes.medium.name;
  ...
  openModalForMagic(template: TemplateRef<any>, data?: any) {
    this.modalRefForMagic = this.modalService.show(template, {
      ignoreBackdropClick: true,
      class: "modal-xl",
    });
  }
  ...
  submit(): void {
    ...
    this.articlesService.updateArticle(object).subscribe(
      (resCreate) => {
  ...
  getBodyWithTags() {
    this.tags.map((t) => {
      ...
      this.article.attributes.body = this.article.attributes.body.replace(
        TAGRe,
  ...
  exportPDF() {
    ...
    this.articlesService.exportPDF(this.articleId).subscribe(
      (res) => {
  ...
  SendMail() {
    if (this.email != "") {
      this.articlesService.sendEmail(this.articleId, this.email).subscribe(
        (res) => {
          this.modalRefEmail.hide();
          this.notifications.create(
            "Success",
    ...
    switch (choise) {
      case "résumé":
        if (this.article.attributes.language == "fr") {
          this.message =
            "donne moi un résumé de cet article: " +
            this.article.attributes.body;
        } else {
          this.message =
            "donne moi un résumé en arabe de cet article: " +
            this.article.attributes.body;
        }
        break;
      case "personnes existantes":
        if (this.article.attributes.language == "fr") {
          this.message =
            "extrais moi les personnes existantes avec nom prénom et leur status de cet text: " +
            this.article.attributes.body;
        } else {
          this.message =
            "extrais moi les personnes existantes avec nom prénom et leur status en arabe de cet text: " +
            this.article.attributes.body;
        }
        break;
      case "chiffres existants":
        if (this.article.attributes.language == "fr") {
          this.message =
            "Extrait moi les chiffres avec labels en colonne ( Lebel, Chiffre ) de ce texte : " +
            this.article.attributes.body;
        } else {
          this.message =
            "Extrait moi les chiffres avec labels en colonne ( Lebel, Chiffre ) en arabe de ce texte : " +
            this.article.attributes.body;
        }
        break;
      case "dates existantes":
        if (this.article.attributes.language == "fr") {
          this.message =
            "extrais moi les dates avec leur événements en colonne de ce texte : " +
            this.article.attributes.body;
        } else {
          this.message =
            "extrais moi les dates avec leur événements en colonne en arabe de ce texte : " +
            this.article.attributes.body;
        }
        break;
    }
  ...
  sendToopenai(choise) {
    this.openai
      .createCompletion(
        { model: "text-davinci-003", prompt: this.message, max_tokens: 1200 },
        {
          headers: {
            Authorization: `Bearer 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`,
          },
        }
      )
      .then(
        (response) => {
          let respons = response?.data?.choices[0]?.text
            .replace(/^\n\n/, "")
            .replace(/\n/g, "<br>");
        ...
        (error) => {
          this.notifications.create(
            ...
            "Merci de refaire la requete pour avoir une meilleure réponse",
