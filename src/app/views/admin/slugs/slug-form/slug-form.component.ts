...
export class SlugFormComponent implements OnInit {
  ...
  constructor(
    ...
    private slugService: SlugsService,
  ...
  createSlug(event) {
    ...
    if (this.slugForm.valid) {
      if (this.data) {
        ...
        this.slugService.updateSlug(object).subscribe(
          (resCreate) => {
            this.notifications.create(
              "Success",
              "Mettre à jour le slug avec succès",
        ...
        this.slugService.addSlug(slug).subscribe(
          (resCreate) => {
            this.notifications.create(
              "Success",
              "Slug créé avec succès",
              NotificationType.Success,
              { theClass: "primary", timeOut: 6000, showProgressBar: false }
            );
            this.modalRef.hide();
