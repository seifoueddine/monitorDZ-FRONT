...
allSlugs: any = [];
  constructor(private modalService: BsModalService,
    private userService: UsersService, private notifications: NotificationsService, private slugsService: SlugsService,
    private ourNotificationService: OurNotificationsService, private langService: LangService) {
  ...
  show(data?) {
    this.data = data
    this.modalRef = this.modalService.show(this.template, this.direction === 'ltr' ? this.configRight : this.configLeft);
    this.createUserForm();
  }
  ...
  createUserForm() {
    if (!this.data) {
      ...
      this.userForm = new FormGroup({
        ...
        email: new FormControl(this.data.attributes.email, [Validators.required, Validators.email]),
    ...
    if (this.userForm.valid) {
      ...
      //  formData.append('first_name', this.userForm.value.first_name);
       ...
       // formData.append('status', this.userForm.value.status);
        this.userService.updateUser(formData , userId
      ).subscribe(
        res =>  {console.log(res)
